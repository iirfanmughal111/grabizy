                            ...
                            <tbody>
                                ...
                                <tr class="align-middle notif-row" id="notif-row-{{$notif->id}}">
                                    ...
                                    <td class="">
                                        ...
                                        @if ($data->status == 0)
                                        <a class="dropdown-item" href="{{url('admin/users/profile/'.$notif->user_id.'?notification='.$notif->id)}}"
                                            id="notif-row-{{$notif->id}}">
                                            <i class="fa-solid fa-user me-2"></i><span>{{$data->msg}}</span></a>
                                        @elseif ($data->status == 1)
                                        <a class="dropdown-item" href="{{url('shop/view-order/'.$array[2].'?notification='.$notif->id)}}"
                                            id="notif-row-{{$notif->id}}">
                                            <i class="fa-solid fa-cart-flatbed me-2"></i><span>{{$data->msg}}</span></a>
                                        @elseif ($data->status == 2)
                                        <a class="dropdown-item" href="{{url('shop/view-order/'.$array[3].'?notification='.$notif->id)}}"
                                            id="notif-row-{{$notif->id}}">
                                            <i class="fa-solid fa-cart-flatbed me-2"></i><span>{{$data->msg}}
                                            </span></a>
                                        @elseif ($data->status == 3)
                                        <a class="dropdown-item" href="{{url('auction/view/'.$array[3].'/aution'.'?notification='.$notif->id)}}"
                                            id="notif-row-{{$notif->id}}">
                                            <i class="fa-regular fa-paper-plane me-2"></i><span>{{$data->msg}}</span></a>
                                            
                                            @elseif ($data->status == 7)
                                        <a class="dropdown-item" href="{{url('wishbox?active=1&wish_id='.$array[7].'?notification='.$notif->id)}}"
                                            id="notif-row-{{$notif->id}}">
                                            <i class="fa-regular fa-paper-plane me-2"></i><span>{{$data->msg}}</span></a>
                                        @endif
                                    ...
                                    <td>
                                        <button data-notif-id="{{$notif->id}}"
                                            class="readNotifications px-3 btn-default btn-sm btn-primary"  {{ $notif->read_at!=null ? 'disabled' : '' }}><i
